...
import { Upload, Download, Type, Image as ImageIcon, Trash2, Sliders, AlignLeft, AlignRight } from 'lucide-react';
...
<div className="min-h-screen bg-slate-100 p-4 md:p-8 font-sans text-slate-900">
    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        {/* Controls Panel */}
        <div
            className="lg:col-span-1 space-y-6 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 overflow-y-auto max-h-[90vh]">
            <div>
            ...
            <div className="space-y-3">
                ...
                <input type="file" ref={fileInputRef} onChange={handleMainImageUpload} className="hidden"
                    accept="image/*" />

                <div className="flex gap-2">
                    <button onClick={()=> logoInputRef.current.click()}
                        className="flex-1 flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200
                        text-slate-700 py-3 rounded-xl transition-all font-semibold border border-slate-200 text-sm"
                        >
                        <Upload size={16} />
            ...
            <div className="space-y-4 border-t border-slate-100 pt-4">
                <h3 className="text-sm font-bold flex items-center gap-2 text-slate-600">
                    <ImageIcon size={14} /> Logo Settings
                </h3>
                <div className="flex bg-slate-100 p-1 rounded-lg">
                    <button onClick={()=> setConfig({...config, logoPosition: 'left'})}
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-xs font-bold
                        transition-all ${config.logoPosition === 'left' ? 'bg-white shadow-sm text-blue-600' :
                        'text-slate-500'}`}
                        >
                        <AlignLeft size={14} /> Left
                    </button>
                    <button onClick={()=> setConfig({...config, logoPosition: 'right'})}
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-xs font-bold
                        transition-all ${config.logoPosition === 'right' ? 'bg-white shadow-sm text-blue-600' :
                        'text-slate-500'}`}
                        >
                ...
                <div>
                    ...
                    <input type="range" min="20" max="90" value={config.logoSize} onChange={e=> setConfig({...config,
            ...
            <div className="space-y-4 border-t border-slate-100 pt-4">
                ...
                <div className="space-y-2">
                    {['companyName', 'email', 'phone', 'website'].map((key) => (
                    <input key={key}
            ...
            <div className="space-y-4 border-t border-slate-100 pt-4">
                <h3 className="text-sm font-bold flex items-center gap-2 text-slate-600">
                    <Sliders size={14} /> Header Styling
                </h3>
                <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase mb-2 block">Transparency</label>
                    <input type="range" min="0.1" max="1" step="0.01" value={config.headerOpacity} onChange={e=>
                    setConfig({...config, headerOpacity: parseFloat(e.target.value)})}
                    className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                    />
                </div>
                <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase mb-2 block">Bar Height</label>
                    <input type="range" min="5" max="25" step="1" value={config.headerHeightPercent} onChange={e=>
                    setConfig({...config, headerHeightPercent: parseInt(e.target.value)})}
                    className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                    />
                </div>
                <div className="flex gap-3">
                    <div className="flex-1">
                        ...
                        <input type="color" value={config.headerColor} onChange={e=> setConfig({...config, headerColor:
                        e.target.value})} className="w-full h-8 rounded border-none cursor-pointer" />
                    </div>
                    <div className="flex-1">
                        <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Text</label>
                        <input type="color" value={config.textColor} onChange={e=> setConfig({...config, textColor:
                        e.target.value})} className="w-full h-8 rounded border-none cursor-pointer" />
                    </div>
                </div>
            </div>
        ...
        <div className="lg:col-span-3 flex flex-col">
            <div
                className="bg-slate-300 rounded-3xl flex-1 flex items-center justify-center overflow-hidden border-8 border-white shadow-2xl relative min-h-[600px]">
                {!mainImage ? (
                <div className="text-center p-12">
                    <div
                        className="bg-white/50 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner">
                ...
                <div className="w-full h-full flex items-center justify-center p-4">
                    <canvas ref={canvasRef} className="max-w-full max-h-[85vh] shadow-2xl rounded-sm object-contain" />
                </div>
                )}
            </div>
            <div className="mt-4 text-center">
                <p className="text-xs text-slate-400 font-medium">Original image quality is preserved. This tool runs
